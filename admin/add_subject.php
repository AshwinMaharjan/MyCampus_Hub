    ...
    if (!empty($subject_name) && !empty($staff_id) && !empty($course_id) && !empty($sem_id)) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($marks_id === null || $marks_id === "") {
                ...
                $stmt = $conn->prepare("INSERT INTO subject (sub_name, marks_id, role_id, course_id, sem_id) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("siiii", $subject_name, $marks_id, $staff_id, $course_id, $sem_id);
            }
            ...
            if ($stmt->execute()) {
                ...
                $notification = "Error: Could not add subject.";
                $notification_type = "error";
            }
            $stmt->close();
        }
...
<head>
  ...
  <style>
    ...
    select:focus {
      border-color: #263576;
      outline: none;
      box-shadow: 0 0 5px rgba(38, 53, 118, 0.5);
    }
    button {
      width: 100%;
      padding: 12px;
    ...
    <form method="POST" action="">
      ...
      <select name="staff_id" required>
        ...
        <?php foreach ($staffList as $staff): ?>
          <option value="<?= $staff['user_id'] ?>"><?= htmlspecialchars($staff['full_name']) ?></option>
        <?php endforeach; ?>
      </select>
      <span class="error" id="staff_error"></span>

      <select name="sem_id" required>
        <option value="" disabled selected>Select Semester</option>
        <?php foreach ($semesterList as $sem): ?>
          <option value="<?= $sem['sem_id'] ?>"><?= htmlspecialchars($sem['sem_name']) ?></option>
        <?php endforeach; ?>
      </select>
      <span class="error" id="sem_error"></span>
...
let existingSubjects = [
    ...
    if ($subjectResult) {
        while ($row = $subjectResult->fetch_assoc()) {
            $subjectNames[] = '"' . strtolower(addslashes($row['sub_name'])) . '"';
        }
    }
...
function showValidationError(message) {
    ...
    overlay.innerHTML = `
        <div class="notification-modal invalid">
            ...
            <div class="notification-title">Duplicate Subject</div>
...
document.querySelector('form').addEventListener('submit', function(e) {
    ...
    if (hasNumbers) {
        e.preventDefault(); // Prevent form submission
        showValidationError('Subject name cannot contain numbers. Please use only letters and spaces.');
        return false;
    }
